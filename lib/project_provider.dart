...
final projectProvider = StateNotifierProvider<ProjectState, Project>((ref) {
  return ProjectState(project: Project(name: "Med. 1", inputs: defaultInputs));
});
...
class ProjectState extends StateNotifier<Project> {
  ...
  void addInput({String? newInput}) {
    state =
        Project(name: state.name, inputs: [...state.inputs, newInput ?? ""]);
  }

  void editInput(int index, String newText) {
    state = Project(
        name: state.name,
        inputs: List<String>.of(state.inputs)
          ..replaceRange(index, index + 1, [newText]));
  }
